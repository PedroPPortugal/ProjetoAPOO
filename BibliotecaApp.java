...
public class BibliotecaApp {
    ...
    private static void adicionarLivro() {
        ...
        if (!isValidISBN(isbn)) {
            System.out.println("Erro: ISBN inválido. Use formato ISBN-10 ou ISBN-13.");
            return;
        }
    ...
    private static boolean isValidISBN(String isbn) {
        ...
        if (cleanISBN.length() == 10) {
            return cleanISBN.substring(0, 9).matches("\\d+") &&
                    (Character.isDigit(cleanISBN.charAt(9)) || cleanISBN.charAt(9) == 'X');
        }
        ...
        if (scanner.hasNextInt()) { // Verifica se a entrada é um número inteiro
            ...
            if (index >= 0 && index < livros.size()) { // Verifica se o índice é válido
                ...
                if (scanner.hasNextInt()) { // Valida o ano de edição
                    ...
                    System.out.println("Entrada inválida para o ano de edição.");
            ...
            switch (opcao) {
                case 1:
                    if (utentes.isEmpty() || livros.isEmpty()) {
                        System.out.println("Não há utentes ou livros registados.");
                    } else {
                        registarEmprestimo();
                    }
                    break;
        ...
        if (scanner.hasNextInt()) {
            ...
            if (utenteIndex >= 0 && utenteIndex < utentes.size()) {
                ...
                if (!livrosEmprestimo.isEmpty()) {
                    ...
                    System.out.println("Nenhum livro foi adicionado ao empréstimo.  ");
                }
            } else {
                System.out.println("Opção inválida!");
            }
